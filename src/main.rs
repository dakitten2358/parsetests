...
fn main() {
    ...
    let config: TestConfiguration =
        toml::from_str(config_toml.as_str()).expect("failed to parse toml");
    ...
    test_pass
        .tests
        .sort_by(|a, b| a.full_test_path.cmp(&b.full_test_path));
            ...
            TestResult::Fail => {
                ...
                for entry in test.entries {
                    match entry.event.entry_type {
                        EntryType::Info => {
                            println!("{}{}{}", empty_spacer, log_info, entry.event.message)
                        }
                        EntryType::Warning => {
                            println!("{}{}{}", empty_spacer, log_warn, entry.event.message);
                            println!(
                                "{}{}{}:{}",
                                empty_spacer, empty_spacer, entry.filename, entry.line_number
                            );
                        }
                        EntryType::Error => {
    ...
    if test_pass.failed > 0 {
        println!(
            ...
            format!(
                ...
                test_pass.succeeded,
                test_pass.failed,
                test_pass.not_run + test_pass.succeeded_with_warnings
            )
            .red()
        );
